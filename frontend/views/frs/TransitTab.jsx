...
import { Badge, Button, Card, ConfirmDialog, EmptyState, Input, Modal, Select, Spinner, Table, Toggle } from "@/web/kit";
...
function Sessions({ rules }) {
  ...
  const counts = useMemo(() => ({ open: list.filter((s) => s.status === "open").length, overdue: list.filter((s) => s.status === "overdue").length, closed: list.filter((s) => s.status === "closed").length }), [list]);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-3">
        {[["", "All"], ["open", "Open"], ["overdue", "Overdue"], ["closed", "Closed"]].map(([k, l]) => (
...
const RULE_EMPTY = { name: "", entry_camera: "", exit_cameras: [], window_minutes: 5, enabled: true };
...
function Rules({ q }) {
  ...
  const camerasQ = useQuery({ queryKey: ["frs-cameras"], queryFn: () => api.get("/frs/cameras").then((r) => r.data) });
  const cameras = camerasQ.data || [];
  const camName = (id) => cameras.find((c) => String(c.id) === String(id))?.name || id || "—";
  const camOptions = cameras.map((c) => ({ value: c.id, label: c.name }));

  const invalidate = () => { qc.invalidateQueries({ queryKey: ["frs-transit-rules"] }); setOpen(false); setEditing(null); setForm(RULE_EMPTY); };
  const toBody = (f) => ({ name: f.name, enabled: f.enabled, config: { entry_camera: f.entry_camera, exit_cameras: (f.exit_cameras || []).filter(Boolean), window_minutes: Number(f.window_minutes) || 0 } });
  ...
  function openEdit(r) { setEditing(r); setForm({ name: r.name || "", entry_camera: r.config?.entry_camera || "", exit_cameras: r.config?.exit_cameras || [], window_minutes: r.config?.window_minutes ?? 5, enabled: r.enabled }); setOpen(true); }
  const saving = create.isPending || patch.isPending;

  const cols = [
    { key: "name", label: "Name", render: (r) => <div className="font-medium">{r.name}</div> },
    { key: "entry", label: "Entry", render: (r) => <span className="text-muted">{r.config?.entry_camera ? camName(r.config.entry_camera) : "—"}</span> },
    { key: "exits", label: "Exits", render: (r) => <span className="text-muted">{(r.config?.exit_cameras || []).map(camName).join(", ") || "—"}</span> },
    { key: "window", label: "Window", render: (r) => <span className="text-muted flex items-center gap-1"><Icon icon="heroicons-outline:clock" />{r.config?.window_minutes || 0}m</span> },
    ...
    <div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit rule" : "New rule"}
        ...
        <div className="space-y-4">
          ...
          <Select label="Entry camera" options={camOptions} value={form.entry_camera} onChange={(e) => setForm({ ...form, entry_camera: e.target.value })} placeholder="Select entry camera" />
          <div>
            <div className="text-sm font-medium text-foreground mb-1.5">Exit cameras</div>
            <div className="rounded-md border border-card-border divide-y divide-card-border max-h-44 overflow-y-auto">
              {cameras.length === 0 ? (
                <div className="px-3 py-2 text-sm text-muted">No cameras available</div>
              ) : cameras.map((c) => {
                const checked = (form.exit_cameras || []).map(String).includes(String(c.id));
                return (
                  <label key={c.id} className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-hover">
                    <input type="checkbox" checked={checked} onChange={(e) => setForm({ ...form, exit_cameras: e.target.checked ? [...(form.exit_cameras || []), c.id] : (form.exit_cameras || []).filter((x) => String(x) !== String(c.id)) })} />
                    <span className="text-foreground">{c.name}</span>
                  </label>
                );
              })}
            </div>
            <div className="text-xs text-muted mt-1">One or more exit cameras</div>
          </div>
          <div className="grid grid-cols-2 gap-4 items-end">
            <Input label="Deadline (minutes)" type="number" value={form.window_minutes} onChange={(e) => setForm({ ...form, window_minutes: e.target.value })} />
