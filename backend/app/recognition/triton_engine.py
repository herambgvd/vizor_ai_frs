...
_EMB_OUT, _AS_OUT = "683", "output"
# This FairFace export has THREE output heads (race[7] / gender[2] / age[9]) rather
# than a single [18] tensor. Request all three and concatenate to the 18-vector
# fairface.postprocess_fairface expects (race[0:7], gender[7:9], age[9:18]).
_FF_OUTS = ["race_output", "gender_output", "age_output"]
...
class TritonEngine:
    ...
    def age_gender(self, crop_bgr: np.ndarray):
        ...
        raw = self._infer(_FF_MODEL, _FF_IN, preprocess_fairface(crop_bgr), _FF_OUTS)
        if not raw:
            return None
        try:
            combined = np.concatenate([
                raw["race_output"].flatten(),
                raw["gender_output"].flatten(),
                raw["age_output"].flatten(),
            ])
        except Exception:  # noqa: BLE001
            return None
        return postprocess_fairface(combined)
